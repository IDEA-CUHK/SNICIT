...
void SNICIT::_infer() {
  ...
  for (int round = 0; round < num_input / batch_size; round++) {
    ...
    sparse_hidden_post<<<batch_size, dim3((int)(1024 / num_hidden_neurons), num_hidden_neurons, 1),
                         num_hidden_neurons * sizeof(float), dev_stream>>>(
        _dev_Y_hidden[cur_layer % 2],
        _dev_hidden_delta_index[cur_layer],
        _dev_hidden_nonzero_values[cur_layer],
        _dev_hidden_minimum[cur_layer],
        _dev_hidden_row_offset[cur_layer],
        _dev_hidden_avg_nnz[cur_layer],
