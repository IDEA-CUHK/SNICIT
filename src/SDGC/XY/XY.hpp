...
{
  std::cout<<("Constructing XY method......")<<std::endl;
}


void XY::_dense_reorder(std::vector<std::vector<float>> &input, Reorder &reorder_class) {
...
int XY::_infer(
    ...
    int feature = 0;
    for(int l = 0; l < layer; ++l) {
        auto stream = env.get_stream("XY-2021");
        env.event_start_record("XY-2021");
        ...
        env.event_stop_record("XY-2021-kernel");
        float time = env.get_event_time("XY-2021-kernel"); 
        if (l >= _threshold) {
            stat_time += time;
            post_conv_cnt++;
        }
	...
	std::cout << "XY info: runtime " << all_time <<  "ms" << 
