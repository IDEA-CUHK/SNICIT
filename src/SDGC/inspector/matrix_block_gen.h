    ...
    class SparseMatrixBlockGen {
        ...
        static MatrixPos rectangels_max(MatrixPos start_pos, CSRCSCMatrix &csr_csc) {
            ...
            for(int i = 0; i < row_max; ++i) {
                now_max_row = i + 1;
                col_max = col_line_succ_max({start_pos.row_idx, start_pos.col_idx + i}, csr_csc);
                // std::cout << "col max = " << col_max <<  ", " << start_pos.row_idx << ", " << start_pos.col_idx + i << std::endl;
                if (col_max < now_max_col) now_max_col = col_max;
                // now_max_col = std::min(col_max, now_max_col);
                tmp_area = now_max_col * now_max_row;
                // std::cout << "now_max_col = " << now_max_col <<  ", tmp_area = " << tmp_area << std::endl;
                if(tmp_area > now_max) {
                    now_max = tmp_area;
                    res_row = now_max_row;
                    res_col = now_max_col;
                    // printf("res_col=%d ", res_col);
                }
                // std::cout << "----res_row = " << res_row <<  ", res_col = " << res_col << std::endl;
            }
            // std::cout << "----res_row = " << res_row <<  ", res_col = " << res_col << std::endl;
